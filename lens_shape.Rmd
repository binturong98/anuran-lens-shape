...
specimen_means <- lens_data %>%
  ...
  group_by(specimen_id, genus_species, family, life_stage, adult_ecology, activity_period) %>%
...
Here, we estimated phylogenetic signal in tadpole lens shape, in adult lens shape, and in the change in lens shape through ontogeny (difference between adults and tadpoles within species). For these analyses, lens shape was represented by the PCA scores from the above analysis, which allowed us to use a multivariate approach. 
...
Here, we examine whether the change in lens shape through ontogeny (difference between tadpole and adult lens shape within species) is correlated with adult habitat. We do this first by examining each 3D lens metric individually, and then with a multivariate approach. 

#### PGLS 

```{r}

#pgls regressions testing for effect of adult habitat on each 3D shape metric (separately) f
...
procD<-procD.pgls(both_phylo_means[,c("mean_anisotropy.diff","mean_elongation.diff","mean_sphericity.diff")]~adult_ecology.adults, tree.both, Cov = NULL, iter = 999,seed = NULL, int.first = FALSE, SS.type = NULL, effect.type = NULL, data = both_phylo_means, print.progress = TRUE)
summary(procD)
...
Here, we examine whether adult lens size (relative to eye size) or shape are correlated with adult ecology (habitat or activity period). We do this across all adults sampled with available data (n = 181 species, with fewer for eye size data)
...
#### Tadpole lens shape vs. adult ecology (DELETE???)

Here is a Procrustes MANOVA of tadpole lens shape (multivariate) across adult habitats. Currently, this is not included in the manuscript. There is no significant effect of adult habitat on tadpole lens shape. 
...
#procD.pgls of differences in tadpole eye shape (all 3 measures) across adulthabitats
procD<-procD.pgls(tad_phylo_means[,c("mean_anisotropy.tads","mean_elongation.tads","mean_sphericity.tads")]~adult_ecology.tads, tree.tad, Cov = NULL, iter = 999,seed = NULL, int.first = FALSE, SS.type = NULL, effect.type = NULL, data = tad_phylo_means, print.progress = TRUE)
summary(procD)
...
adult_eye_eco <- filter(adult_eye, adult_ecology != "unknown") %>% droplevels()
...
## Figure 3 : Phylogeny with PC scores. 
...
#Export figure
pdf(file = "Figures/Fig3.pdf", width = 6,  height = 12)
plot(tree.pruned, show.tip.label= F, font=3, cex=0.5, label.offset = 1.5, edge.width=3, x.lim=c(0,700))
par(fg="black")
tt<-gsub("_"," ",tree.pruned$tip.label)
text(lastPP$xx[1:length(tt)],lastPP$yy[1:length(tt)], tt,cex=0.5,col=SemiEcoColr, pos=4, offset=0.1, font=3)
...
## Figure 4 - Original version: PCA of tadpoles and adults coloured by microhabitat, violins of shape metrics by tadpole and adult
...
## Figure 4 - New version: Species with matched tadpole and adult lens shape data across adult habitats. 

```{r Figure 4new}

#### Violin of shape metrics by paired tadpole and adult ####
...
#anisotropy
an.paired.plot <- ggplot(matchData, aes(x = stage, y = anisotropy, col = eco)) +
  scale_color_manual(values = col_hab, name = "Adult habitat") +
  scale_shape_manual(values = shape_stage, name = "Life stage") +
...
#sphericity
sp.paired.plot <- ggplot(matchData, aes(x = stage, y = sphericity, col = eco)) +
  scale_color_manual(values = col_hab, name = "Adult habitat") +
  scale_shape_manual(values = shape_stage, name = "Life stage") +
  geom_point(size = 2, alpha = 0.9, aes(shape = stage)) + 
  geom_line(aes(group = species, col = eco), alpha = 0.5) +
...
#elongation
el.paired.plot <- ggplot(matchData, aes(x = stage, y = elongation, col = eco)) +
  scale_color_manual(values = col_hab, name = "Adult habitat") +
  scale_shape_manual(values = shape_stage, name = "Life stage") +
  geom_point(size = 2, alpha = 0.9, aes(shape = stage)) + 
  geom_line(aes(group = species, col = eco), alpha = 0.5) +
...
plots <- plot_grid(an.paired.plot, sp.paired.plot,
           align = 'vh', #align horizontally and vertically
           axis = 'lb',
...
#export figure
pdf(file = "Figures/Fig-supp-paired.pdf", width = 8,  height = 6)
el.paired.plot
dev.off()
```


## Figure 5: Adult lens shape by ecology
...
#print graph
pdf(file = "Figures/Fig5AB.pdf", width = 12,  height = 5, useDingbats=FALSE)
par(mfrow=c(1,2))
ecoPlot<-pca2d(eco.pca$x, group= Eco, shape = 16 , col =EcoColr, show.centroids = TRUE, radius =1.5,show.labels=F)
activPlot<-pca2d(activ.pca$x, group= activ, shape = 16 , col =activColr, show.centroids = TRUE, radius =1.5,show.labels=F)
dev.off()
...
#adjust naming of categories
adult_phylo_activ <- adult_phylo_activ %>%
...
fig.c <- anis_habs + theme(legend.position = "none")
fig.e <- sph_habs + theme(legend.position = "none")
fig.d <- anis_act + theme(legend.position = "none")
fig.f <- sph_act + theme(legend.position = "none")
...
#export figure
pdf(file = "Figures/Fig5cdef.pdf", width = 12,  height = 8)
plots
dev.off()
...
#export figure
pdf(file = "Figures/Fig-supp-adultelong.pdf", width = 12,  height = 4)
plots
dev.off()
```


## Figure 6: Violins of relative lens size by microhabitat (Panel A) and activity pattern (Panel B)
...
hab.eye.pgls <- pgls(mean.lens.diameter ~ mean.eye.diameter,
                       data = hab.eye.comp, 
                       lambda="ML")


#extract pgls residuals 
pglsres.habeye <- residuals(hab.eye.pgls) 
...
gg_eco<-ggplot(adult_eye_eco, aes(x=adult_ecology, y=pglsres.habeye, fill=adult_ecology)) + 
  geom_violin(trim=FALSE, alpha = 0.5)+
  scale_fill_manual(values = col_hab, name = "Adult habitat") +
...
#fit pgls model for lens vs. eye size
act.eye.pgls <- pgls(mean.lens.diameter ~ mean.eye.diameter,
                       data = act.eye.comp, 
                       lambda="ML")

#extract pgls residuals 
pglsres.acteye <- residuals(act.eye.pgls) 
...
plots <- plot_grid(gg_eco + theme(legend.position = "none"), 
                   gg_activ + theme(legend.position = "none"),
           align = 'vh', #align horizontally and vertically
           axis = 'lb',
           labels = c("A", "B"), #panel labels for figure
           hjust = -.1, #adjustment for panel labels
...
#print graph
pdf(file = "Figures/Fig6.pdf", width = 12,  height = 4)
plots
dev.off()
