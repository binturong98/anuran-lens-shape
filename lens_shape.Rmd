...
plot_pca <- ggplot(df_pca, 
  ...
  stat_ellipse(data = filter(df_pca, group == "tadpole"), aes(x=PC1, y=PC2), type = "norm") + # adds elliptical cluster for tadpole group
...
#note for the arros, the one pointing left is for anisotropy, up is for flatness, to the right is for sphericity. 
...
fig.c <- anis_habs + 
  ...
  ylab("Anisotropy") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_hline(yintercept = specimen_means$mean_anisotropy[specimen_means$mean_anisotropy == "Lens_replica"], linetype = "dotted") +
