...
elong.plot <- shape.final %>%
  ...
  geom_violin(alpha=0.1, trim = T, width = 1, position = position_dodge(1)) + 
  geom_boxplot(width = 0.2, position = position_dodge(1), alpha=0.1) + #controls boxes
  theme(text = element_text(size=12), panel.background = element_blank(), axis.line = element_line(colour = "black")) + #controls background
  guides(fill = FALSE) + #removes the life stage legend
  xlab("") +
...
#export figure
pdf(file = "Figures/Fig2.pdf", width = 12,  height = 8)
grid.arrange(plot_top, plots_bottom, ncol = 1, nrow = 2)
dev.off()
```
