...
The following subset includes one row for **each specimen** measured. We created this subset by removing repeated measurements of the same lens, and then calculating the mean of the left and right lens for each specimen. This subset also includes outgroups and reference blender shapes. 
```{r specimen-means}
...
species_stage_means <- specimen_means%>%
  filter(life_stage != "outgroup" & life_stage != "artificial") %>%
...
LR_lens_data <- lens_data %>%
  mutate(genus_species = as.factor(paste(genus, species, sep = "_"))) %>% # creates a genus_species column
  filter(life_stage != "repeat" & specimen_id != "BMNH1906.8.30.62" & life_stage != "outgroup" & life_stage != "artificial") %>%
  mutate_if(is.character, as.factor) %>% 
...
# Figure 2 : Phylogeny with PC scores. Species names are coloured by microhabitat. Circles and dotted lines added in illustrator
...
# Figure 3: PCA of tadpoles and adults coloured by microhabitat and violins of each of the 3 shape metrics by tadpole and adult
...
# Figure 4: Violins of relative lens size by microhabitat (Panel A) and activity pattern (Panel B)
...
#print graph
pdf(file = "Fig4_RelLensSize_.pdf", width = 6,  height = 8)
