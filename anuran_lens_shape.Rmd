...
counts <-ddply(lens_data, .(lens_data$family, lens_data$genus, lens_data$species, lens_data$life_stage, lens_data$adult_ecology), nrow)
names(counts) <- c("Family","Genus","Species", "Stage", "Adult ecology", "Lenses sampled")

#create scrolling RMarkdown table of sampling
kable(counts[ , c("Family","Genus","Species", "Stage", "Adult ecology", "Lenses sampled")], caption = "Species and sampling effort for CT scan data.") %>%
...
# Calculating the mean of the right and left lenses

```{r specimen-means of entire dataset}
...
# Create subset of species with data for both tadpoles and adults

```{r subset for adult-tadpole matches only}
...
```{r subset for adult-tadpole matches in wide format}

#merge tadpole and adult dataframes keeping only matching species
...
```{r subset for only anura & subseet for only adult anura}
...
# Create dataframe with one row per species, with mean data for tadpoles and adults in separate columns

```{r species-means for anura only in wide format}
...
# Import a published phylogeny and match to dataset
...
### Match data species names with phylogeny ###
...
#rename 
lens.tree <- ladderize(tree.pruned) #ladderizes tree
...
```{r bar plot: relative percentage change: anisotropy ~ life stage, group = ecology}
...
```{r boxplot: relative percentage change: anisotropy  ~ life stage, group = ecology}
...
```{r paired boxplot: flatness ~ tadpoles, adults, group = ecology}       
...
```{r paired boxplot: anisotropy ~ tadpoles, adults, group = ecology}       
...
```{r paired boxplot: sphericity ~ tadpoles, adults, group = ecology}       
...
```{r paired boxplot: elongation ~ tadpoles, adults, group = ecology}       
...
```{r 3d plot: flatness, anisotropy, sphericity, group = ecology}
...
```{r 3d plot: flatness, anisotropy, sphericity, group = life stage}
plot3d <- plot_ly(lens_mean, 
                  x = ~ mean_flatness, 
                  y = ~ mean_sphericity,
                  z = ~ mean_anisotropy,
