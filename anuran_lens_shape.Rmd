...
specimen_means <- lens_data %>%
  ...
  filter(life_stage != "repeat" & life_stage != "outgroup" & specimen_id != "BMNH1906.8.30.62") %>%
  mutate_if(is.character, as.factor) %>% 
  group_by(specimen_id, genus_species, family, life_stage, adult_ecology, adult_ecology_2, activity_period, scan_source, stain) %>%
...
tads <- filter(specimen_means, life_stage == "tadpole")
adults <- filter(specimen_means, life_stage == "adult")

#find means for replicates of life stage within species
#calculate mean lens parameters for each specimen
tads_mean <- tads %>%
  group_by(genus_species, family, adult_ecology, adult_ecology_2, activity_period) %>%
...
The following dataframe extracts only the species for which we had scans sampled for **both the tadpole and the adult stage**. It is subset from the dataframe of mean lens shapes by species and life stage.  

```{r adult-tadpole matches}

# Adult-tad species matches in wide format -----

#merge tadpole and adult mean dataframes keeping only matching species (in wide format)
lens_matched <- inner_join(tads_mean, adults_mean, 
             by = "genus_species", 
             suffix = c(".tads", ".adults"))


# Adult-tad species matches in long format -----

#subset species with no tad/adult match from species means by life stage
unmatch <- c(setdiff(tads_mean$genus_species, adults_mean$genus_species), setdiff(adults_mean$genus_species, tads_mean$genus_species))

#add life stage column to adult and tad mean dataframes
adults_mean$life_stage <- "adult"
tads_mean$life_stage <- "tadpole"

#merge tadpole and adult mean dataframes by species and remove unmatched species from dataframe (umatched tad/adult)
lens_matched_long <- merge(tads_mean, adults_mean, all = TRUE) %>%
 filter(!(genus_species %in% unmatch))
            
```

## Subset of adult specimeen lens shape means

This subset removed tadpoles, as well as artificial objects, from **specimen means** for some of Amartya's analyses and plots. 

```{r adult-anura}

# removing artifical objects and outgroups from the dataset
anura_mean <- specimen_means %>% 
  filter(life_stage != "artificial" & life_stage != "outgroup")

# subset with only adults 
adult_mean <- specimen_means %>% 
...
```{r phylo-import, fig.height = 18, fig.width=7, results = FALSE}
...
## Phylogenetic distribution of sampling

```{r phylo-sampling, fig.height = 18, fig.width = 9, results="hide"}
...
## Distribution of shape data across full dataset
...
## Lens shape parameters for tadpoles vs. adults (full dataset)
...
plot_flat <- ggplot(data = filter(specimen_means, life_stage != "outgroup", specimen_id != "Sphere", specimen_id != "Oblate_spheroid", specimen_id != "Prolate_spheroid", specimen_id != "Lens_replica"), #removes outgroups, artifical objects
  ...
  xlab("Life Stage") +
  ylab("Flatness")  +
 geom_hline(aes(yintercept = specimen_means$mean_flatness[specimen_means$specimen_id == "Sphere"]), linetype =   "dashed") +
  geom_hline(aes(yintercept = specimen_means$mean_flatness[specimen_means$specimen_id == "Lens_replica"]), linetype = "dotted")
...
plot_sphericity <- ggplot(data = filter(specimen_means, life_stage != "outgroup", specimen_id != "Sphere", specimen_id != "Oblate_spheroid", specimen_id != "Prolate_spheroid", specimen_id != "Lens_replica"),
  ...
  xlab("Life Stage") +
  ylab("Sphericity") +
 geom_hline(aes(yintercept = specimen_means$mean_sphericity[specimen_means$specimen_id == "Sphere"]), linetype =   "dashed") +
  geom_hline(aes(yintercept = specimen_means$mean_sphericity[specimen_means$specimen_id == "Lens_replica"]), linetype = "dotted")
...
plot_anisotropy <- ggplot(data = filter(specimen_means, life_stage != "outgroup", specimen_id != "Sphere", specimen_id != "Oblate_spheroid", specimen_id != "Prolate_spheroid", specimen_id != "Lens_replica"),
  ...
  xlab("Life Stage") +
  ylab("Anisotropy") +
 geom_hline(aes(yintercept = specimen_means$mean_anisotropy[specimen_means$specimen_id == "Sphere"]), linetype =   "dashed") +
 geom_hline(aes(yintercept = specimen_means$mean_anisotropy[specimen_means$specimen_id == "Lens_replica"]), linetype = "dotted")
...
plot_elongation <- ggplot(data = filter(specimen_means, life_stage != "outgroup", specimen_id != "Sphere", specimen_id != "Oblate_spheroid", specimen_id != "Prolate_spheroid", specimen_id != "Lens_replica"),
  ...
  xlab("Life Stage") +
  ylab("Elongation") +
  geom_hline(aes(yintercept = specimen_means$mean_elongation[specimen_means$specimen_id == "Lens_replica"]), linetype = "dotted") + 
 geom_hline(aes(yintercept = specimen_means$mean_elongation[specimen_means$specimen_id == "Sphere"]), linetype = "dashed")
...
```{r Figure 6, fig.height=8, fig.width=8, results = "hide"}
...
# Comparing life stages across just species with both tadpole and adult sampling

## Relative change in shape from tadpole to adult

These plots show how much anisotropy changed between the tadpole and the adult life stages within species, across adult habitats. 

**Amartya: why if these are relative changes does the y axis go to 600? What is the bar plot showing vs. the box plot, and why do the patterns look different for the same metric?**
...
## Shapes of tadpole vs. adult lenses across adult habitats

These plots show data (subset to the species with matched tadpole and adult lens shape sampling) for tadpole shape and adult shape plotted across adult habitats. 

### Tadpole/adult change in flatness

```{r paired boxplot: flatness ~ tadpoles-adults-ecology}       

flat_box_pair <-  ggplot(data = lens_matched_long,
...
### Tadpole/adult change in anisotropy

```{r paired boxplot: anisotropy ~ tadpoles-adults-ecology}       
aniso_box_pair <-  ggplot(data = lens_matched_long,
...
### Tadpole/adult change in sphericity

```{r paired boxplot: sphericity ~ tadpoles-adults-ecology}       
spher_box_pair <-  ggplot(data = lens_matched_long,
...
### Tadpole/adult change in elongation

```{r paired boxplot: elongation ~ tadpoles-adults,-ecology}       

elong_box_pair <-  ggplot(data = lens_matched_long,
...
### Tadpole/adult change in shape figure

```{r Figure 7, fig.height=8, fig.width=12, results = "hide"}
...
# Adult lens shape across ecological traits

## Habitat

These plot shows adult habitat assigned using the same coding as Thomas et al. anuran eye size paper. 

### Flatness across habitats
...
### Effect of fewer habitat bins

This plot collapses "ground_dwelling", "subfossorial" and "scansorial" species into a single category: "terrestrial"
...
plot_flat_2 <- ggplot(data = filter(anura_mean, life_stage != "tadpole", specimen_id != "DG_frog_1_01"),
  ...
  xlab("Ecology") +
  ylab("Flatness") +
  geom_hline(aes(yintercept = specimen_means$mean_flatness[specimen_means$specimen_id == "Prolate_spheroid"]), linetype = "dotted") + 
  geom_hline(aes(yintercept = specimen_means$mean_flatness[specimen_means$specimen_id == "Oblate_spheroid"]), linetype = "dashed") + 
 geom_hline(aes(yintercept = specimen_means$mean_flatness[specimen_means$specimen_id == "Sphere"]), linetype = "dotdash")

ggplotly(plot_flat_2)
```

### Figure of adult lens shape across habitats

```{r Figure 8, fig.height=8, fig.width=8, results = "hide"}
...
These plots show how adult lens shape varies across adult activity patterns (coded the same way as Thomas et al. eye size paper)

### Flatness across activity periods

```{r adults: flatness ~ activity period}

specimen_means_ap <- specimen_means %>%
  filter(activity_period != "" & activity_period != "unknown")

flat_ap <- ggplot(data = filter(specimen_means_ap, life_stage != "outgroup", life_stage != "tadpole", specimen_id != "Sphere", specimen_id != "Oblate_spheroid", specimen_id != "Prolate_spheroid", specimen_id != "Lens_replica", activity_period != "unknown"),
...
Lens size relative to eye size affects the sensitivity of the eye. In terrestrial species with spherical lenses, myopia is expected, and having a large lens relative to eye size allows the spherical lens to focus on the retina with a short focal length that increases sensitivity (so we may expect this in dim-light environments, like nocturnal or fossorial)

## Scaling of lens size with eye size in adult anurans
...
## Relative lens size across activity periods
...
## Relative lens size across adult habitats
...
## Figure for relative lens size

```{r Figure 9, fig.height=12, fig.width=5, results = "hide"}
